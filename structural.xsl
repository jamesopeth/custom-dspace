        ...
        <html>
            ...
            <body>
                ...
                <div id="ds-main">
                    ...
                    <!-- <xsl:apply-templates /> -->
		    <!-- Las siguientes lineas son adicionadas por james 2014/12/13: Se selecciona el orden en que se imprimen las secciones -->
            ...
            <title>
                <xsl:choose>
                    <xsl:when test="starts-with($request-uri, 'page/about')">
                        <xsl:text>Vitela repositorio Pontificia Universidad Javeriana Cali - Acerca de este repositorio</xsl:text>
                    </xsl:when>
		    <xsl:when test="$request-uri=''">
                        <xsl:text>Vitela repositorio Pontificia Universidad Javeriana Cali</xsl:text>
                    </xsl:when>
        ...
        <div id="ds-header">
			...
			<div id="ds-trail-wrapper">				
				<ul id="ds-trail">
					<xsl:choose>
							<xsl:when test="starts-with($request-uri, 'page/about')">
								<xsl:text>Acerca del repositorio institucional</xsl:text>
							</xsl:when>
    ...
    <xsl:template match="dri:body">
        <div id="ds-body">
            ...
            <xsl:choose>
                <xsl:when test="starts-with($request-uri, 'page/about')">
                    <div>
                        <h1>Acerca de este repositorio</h1>
			<p>El Repositorio Institucional de la Pontificia Universidad Javeriana, se encarga de preservar, difundir y brindar acceso al contenido electrónico producido por la Universidad en sus ambitos administrativo, académico e investigativo. Con el fin de impulsar la apropiación del conocimiento a través de iniciativas de acceso abierto.</p>
                    </div>
                </xsl:when>
                <!-- Otherwise use default handling of body -->
                <xsl:otherwise>
					<xsl:if test="$request-uri=''">
						<!-- <p id="ds-texto-intro">El Repositorio Institucional de la Pontificia Universidad Javeriana, se encarga de preservar, difundir y brindar acceso al contenido electrónico producido por la Universidad en sus ambitos administrativo, académico e investigativo. Con el fin de impulsar la apropiación del conocimiento a través de iniciativas de acceso abierto.</p> -->
						<div id="ds-search-option" class="ds-option-set">
							...
							<form id="ds-search-form" method="post">
								...
								<fieldset>
									<input class="ds-text-field "  placeholder="Nueva búsqueda" type="text">
        ...
        <div id="ds-options">
            ...
            <xsl:if test="count(/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='feed']) != 0">                
                <div id="ds-feed-option" class="ds-option-set">
					<div class="ds-option-set-head-wrapper">
...
-->
    ...
    <xsl:template match="dri:list[@type='form']/dri:item" priority="3">
        <li>
        	...
        	<!--
                Adicionado por James Payan: 2014/12/13: Para dar manejo al listado de búqueda
        	-->
		<xsl:when test="ancestor::dri:list[@n='primary-search'] and not(count(dri:field[@id='aspect.discovery.SimpleSearch.field.query']) = 0)">
                       	<div id="ds-big-search-controls">
				<xsl:apply-templates />
			</div>
                </xsl:when>
		<xsl:when test="ancestor::dri:list[@n='primary-search'] and not(count(dri:xref[@target='display-filters']) = 0)">				
                	<xsl:text></xsl:text>
                </xsl:when>
                ...
                <!--<xsl:apply-templates select="dri:help" mode="compositeComponent"/>-->
                    ...
                    <xsl:otherwise>
                        <input>
                            ...
                            <xsl:attribute name="value">
                                <xsl:choose>
					<!-- para que no se muestre texto en el boton, cuando se está en una página de búsqueda -->
				    <xsl:when test="@id = 'aspect.discovery.SimpleSearch.field.submit'">
					<xsl:text>xmlui.general.go.none</xsl:text>
				    </xsl:when>
                                    <xsl:when test="./dri:value[@type='raw']">
                                        <xsl:value-of select="./dri:value[@type='raw']"/>
                                    </xsl:when>
...
<div class="pagination-controls-wrapper">
	        <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a class="previous-page-link disabled">
                            <i18n:text>xmlui.dri2xhtml.structural.pagination-previous</i18n:text>
                        </a>
                    </xsl:otherwise>
			</xsl:choose>
	        <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <a class="next-page-link disabled">
                            <i18n:text>xmlui.dri2xhtml.structural.pagination-next</i18n:text>
                        </a>
                    </xsl:otherwise>
			</xsl:choose>
...
-->
    ...
    <xsl:template name="itemSummaryList">
        ...
        <xsl:variable name="metsDoc" select="document($externalMetadataUrl)"/>
<div class="ds-artifact-item-wrapper">
        <div class="artifact-description">
			<table>
				<tbody>
					<tr>
						<td class="itemSummaryList-label">
							<i18n:text>xmlui.dri2xhtml.METS-1.0.item-title</i18n:text>:
						</td>
						<td class="itemSummaryList-value">
							<xsl:element name="a">
								<xsl:attribute name="href">
									<xsl:choose>
					...
					<xsl:if test="dri:list[@n=(concat($handle, ':dc.type.spa'))]">
						<tr>
							<td class="itemSummaryList-label">
								<i18n:text>xmlui.dri2xhtml.METS-1.0.item-type</i18n:text>:
							</td>
							<td class="itemSummaryList-value">
								<xsl:apply-templates select="dri:list[@n=(concat($handle, ':dc.type.spa'))]/dri:item"/>
							</td>
						</tr>
					</xsl:if>
					<xsl:if test="dri:list[@n=(concat($handle, ':dc.date'))]">
						...
						<tr>
							...
							<td class="itemSummaryList-value">
								<span class="date">
									<xsl:value-of
										select="substring(dri:list[@n=(concat($handle, ':dc.date'))]/dri:item,1,10)"/>
								</span>
							</td>
						</tr>
	...
	<div class="ds-artifact-preview-wrapper">
    ...
    <xsl:template name="languageSelection">
		<xsl:if test="not(contains($current-uri, '/submit/'))">
			<xsl:if test="count(/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='page'][@qualifier='supportedLocale']) &gt; 1">
				<span class="ds-language-selector" data-dropdown="#ds-language-selection">
				<i18n:text>xmlui.general.language_selector</i18n:text>
				</span>
					<div id="ds-language-selection" class="dropdown dropdown-tip">
				<ul class="dropdown-menu">
							<xsl:for-each select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='page'][@qualifier='supportedLocale']">
									<xsl:variable name="locale" select="."/>
									<li><a>
										<xsl:attribute name="href">
												<!-- <xsl:value-of select="$current-uri"/> --> <!-- se cambia esta linea para colocar la variable $context-path -->
												<xsl:value-of select="$context-path"/>
									...
									<!-- Las líneas a continuación se utilizaron para hacer pruebas con la variable $context-path -->
									<!-- <li><a>
										<xsl:attribute name="href">
