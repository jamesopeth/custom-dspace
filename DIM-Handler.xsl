...
<xsl:stylesheet 
        ...
        <div class="artifact-description">
			<table>
				<tbody>
					...
					<xsl:if test="dim:field[@element='type' and @qualifier = 'spa']">
						<tr>
							<td class="itemSummaryList-label">
								<i18n:text>xmlui.dri2xhtml.METS-1.0.item-type</i18n:text>:
							</td>
							<td class="itemSummaryList-value">
								<xsl:value-of select="dim:field[@mdschema='dc' and @element='type' and @qualifier = 'spa'][1]/node()"/>
							</td>
						</tr>
					</xsl:if>
					<xsl:if test="dim:field[@element='date' and not(@qualifier)]">
						...
						<tr>
							...
							<td class="itemSummaryList-value">
								<span class="date">
									<xsl:value-of select="substring(dim:field[@element='date' and not(@qualifier)]/node(),1,10)"/>
								</span>
							</td>
						</tr>
    ...
    <xsl:template name="communitySummaryList-DIM">
        <xsl:variable name="data" select="./mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim"/>
		<xsl:variable name="logo" select="./mets:fileSec/mets:fileGrp/mets:file"/>
		<a href="{@OBJID}">
			<div>
				<div>
					<xsl:choose>
						<xsl:when test="string-length($logo/mets:FLocat/@xlink:href) &gt; 0">
							<xsl:attribute name="class">
								<xsl:text>logo-community</xsl:text>
							</xsl:attribute>
							<img>
								<xsl:attribute name="src">
									<xsl:value-of select="$logo/mets:FLocat/@xlink:href"/>
								</xsl:attribute>
						...
						<xsl:otherwise>
							<xsl:attribute name="class">
								<xsl:text>no-logo-community</xsl:text>
							</xsl:attribute>
							<img>
								<xsl:attribute name="src">
									<xsl:text>/themes/ri-javeriana/images/no-logo.jpg</xsl:text>
								</xsl:attribute>
								<xsl:attribute name="alt">
									<xsl:text>no-logo</xsl:text>
								</xsl:attribute>
							</img>
						</xsl:otherwise>
					</xsl:choose>
